 ...
 * atomic so both can run concurrently. Set RP_INLINE_WORKER=0 on the web
...
const ctx: AppContext = {
  ...
  config,
  log: {
    info: (m) => console.log(`[readport-worker] ${m}`),
    warn: (m) => console.warn(`[readport-worker] ${m}`),
    error: (m) => console.error(`[readport-worker] ${m}`),
  },
};
