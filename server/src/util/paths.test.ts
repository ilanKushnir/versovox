...
beforeAll(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'tl-paths-'));
  outside = fs.mkdtempSync(path.join(os.tmpdir(), 'tl-outside-'));
...
describe('resolveWithin', () => {
  ...
  it('rejects prefix-sibling escapes (root vs root-suffix)', () => {
    // /tmp/tl-paths-x must not authorize /tmp/tl-paths-x-evil
