 ...
 *
 * Sources, in order:
 *   1. TL_SETUP_TOKEN / TL_SETUP_TOKEN_FILE (recommended; set it in .env).
...
export function ensureSetupToken(
  ...
  if (config.setupToken) {
    ...
    source = 'env';
    log.info('First-run setup is locked with TL_SETUP_TOKEN.');
  } else {
    ...
    log.warn(
      `First-run setup token (required to create the admin account): ${token}\n` +
        `    Also stored at ${tokenPath}. Set TL_SETUP_TOKEN in .env to choose your own.`,
    );
  }
