...
| `VX_JOB_CONCURRENCY`       | `2`                                | Simultaneous **transcriptions** (1–8). Two more slots are always free: one for model downloads, one for scans/indexing/pairing, so a multi-hour alignment never blocks the library.                     |
